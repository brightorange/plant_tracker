...
def plants_info(plant_list):
    """Return as a string the information table of plants with name, location, dates, interval, water need, and water status."""

    plants_info = f"""Name                 Location             Last Watered    Interval (days)  Next Watering   Water Need   Status
{"-" * 120}\n"""

    for p in plant_list:
        next_date = next_watering_date(p)
        next_str = str(next_date) if next_date else "N/A"
        water_need = p.get("water_need", "N/A")

        if need_water(p):
            overdue = overdue_days_since_need_water(p)
            status = f">>> WATER NOW ({overdue} days overdue) <<<"

        plants_info += f"{p['name']:<20} {p['location']:<20} {p['last_watered']:<15} {str(p['water_interval_days']):<16} {next_str:<15} {water_need:<12} {status}\n"
